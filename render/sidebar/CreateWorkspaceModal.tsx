...
export function CreateWorkspaceModal({ onSubmit, onClose }: Props) {
  ...
  const submit = async () => {
    ...
    setCreating(true);
    try {
      onSubmit(name.trim() || "untitled", cwd.trim() || "~/");
    } finally {
      setCreating(false);
    }
  };
    ...
    >
      <div
        ...
        style={{
          ...
          animation: "sbFadeUp .16s cubic-bezier(.2,1,.4,1)",
          overflow: "hidden",
        }}
      >
        {/* Header */}
        ...
        >
          <span style={{ fontSize: FS.base, fontWeight: 600, color: C.t0, fontFamily: SANS }}>
            New workspace
          </span>
          <button
            onClick={onClose}
        ...
        <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Field label="Directory">
            <div style={{ display: "flex", gap: 6 }}>
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    const d = await invoke<string | null>("open_directory_dialog");
                    if (d) setCwd(d);
                  } catch {}
                }}
                style={{
          ...
          <button
            ...
            onMouseEnter={(e) => {
              if (!creating) (e.currentTarget as HTMLElement).style.opacity = ".87";
            }}
            onMouseLeave={(e) => {
