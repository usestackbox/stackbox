...
function restoreUrl(runboxId?: string): string {
  ...
  try {
    return localStorage.getItem(`sbx-browser-url-${runboxId}`) || "http://localhost:3000";
  } catch {
...
export function BrowserPanel({
  ...
  useEffect(() => {
    ...
    if (runboxId)
      try {
        localStorage.setItem(`sbx-browser-url-${runboxId}`, pendingUrl);
      } catch {}
    resetAutoClose();
      ...
      >
                ...
                <BrowsePane
                  ...
                  onUrlChange={(url) => {
                    ...
                    if (runboxId && (url.includes("localhost") || url.includes("127.0.0.1"))) {
                      try {
                        localStorage.setItem(`sbx-browser-url-${runboxId}`, url);
                      } catch {}
                    }
                    resetAutoClose();
                  }}
        ...
        >
          <svg
            ...
            height="10"
            viewBox="0 0 10 10"
            style={{ position: "absolute", bottom: 3, right: 3, opacity: 0.25 }}
          >
