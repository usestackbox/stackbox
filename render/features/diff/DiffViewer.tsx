...
import { HLJS_DIFF_THEME, extColor, getDiffLanguage, parseDiff } from "./diffUtils";
...
export function DiffViewer({ tab, allTabs, onClose, onSelectTab }: Props) {
  ...
  const badgeColor =
    tab.changeType === "deleted"
      ? "rgba(248,113,113,0.8)"
      : tab.changeType === "created"
        ? "rgba(74,222,128,0.7)"
        : "#666666";
      ...
      >
        ...
        {(tab.insertions > 0 || tab.deletions > 0) && (
          <span style={{ display: "flex", gap: 6, flexShrink: 0, fontSize: 10, fontFamily: MONO }}>
            {tab.insertions > 0 && (
              <span style={{ color: "rgba(74,222,128,0.7)" }}>+{tab.insertions}</span>
            )}
            {tab.deletions > 0 && (
              <span style={{ color: "rgba(248,113,113,0.7)" }}>-{tab.deletions}</span>
            )}
          </span>
        )}
      </div>
