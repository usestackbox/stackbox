    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div>
        ...
        <p style={{ margin: 0, fontSize: 13, color: C.t3, lineHeight: 1.6 }}>
          MCP servers give your AI agent access to external tools and data sources. This step is optional.
        </p>
      </div>
      ...
      {!added ? (
        <>
          {/* Presets */}
          <div>
            <p style={{ margin: "0 0 8px", fontSize: 11, color: C.t3, textTransform: "uppercase", letterSpacing: ".07em" }}>
              Quick connect
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {PRESETS.map(p => (
                <button
                  ...
                  style={{
                    padding: "5px 12px", fontSize: 12, fontFamily: SANS,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{
            ...
            <div style={{ fontSize: 11, color: C.t3, fontFamily: MONO, marginTop: 2 }}>{added.url}</div>
          </div>
          <button onClick={() => onNext(added)} style={primaryBtn}>Continue →</button>
        </div>
      )}
    </div>
  );
...
const inputStyle: React.CSSProperties = {
  ...
  padding: "8px 10px", fontSize: 13, fontFamily: SANS,
