    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div>
        ...
        <p style={{ margin: 0, fontSize: 13, color: C.t3, lineHeight: 1.6 }}>
          Docker is used to sandbox agent processes. It's optional — agents can run
          directly on your machine without it.
        </p>
      </div>
      ...
      {status === "missing" && (
        <div style={{
          ...
          borderRadius: 8, fontSize: 12, color: C.amber, lineHeight: 1.7,
        }}>
          Docker wasn't detected. You can still use Stackbox without it — or install it
          now for sandboxed agents.
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {status === "missing" && (
          <>
            <button onClick={openDockerInstall} style={ghostBtn}>Install Docker ↗</button>
            <button onClick={check} style={ghostBtn}>Retry</button>
          </>
        )}
