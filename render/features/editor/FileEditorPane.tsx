...
import { FindBar, type FindMatch } from "./FindBar";
import { LiveEditor } from "./LiveEditor";
import { StatusBar } from "./StatusBar";
import { type HljsCore, extToLang, getHljs } from "./hljs";
  ...
  return (
    <button
      ...
      style={{
        ...
        color: active ? "#00e5ff" : C.t1,
        transition: "all .1s",
      }}
      onMouseEnter={(e) => {
        if (!disabled) {
          const el = e.currentTarget as HTMLElement;
          el.style.background = active ? "rgba(0,229,255,.15)" : C.bg3;
          el.style.color = active ? "#00e5ff" : C.t0;
        }
      }}
      onMouseLeave={(e) => {
        const el = e.currentTarget as HTMLElement;
        el.style.background = active ? "rgba(0,229,255,.15)" : "transparent";
        el.style.color = active ? "#00e5ff" : C.t1;
      }}
    >
      {children}
    </button>
...
export function FileEditorPane({ path, onClose, style }: Props) {
  ...
  const prevMatch = useCallback(
    () =>
      setActiveMatch((v) =>
        findMatches.length ? (v - 1 + findMatches.length) % findMatches.length : 0
      ),
    [findMatches.length]
  );
  ...
  if (error)
    return (
      <div style={placeStyle}>
        ...
        <span style={{ fontSize: 11, color: C.t3, maxWidth: 320, textAlign: "center" }}>
          {error}
        </span>
      </div>
    );
      ...
      >
        <span
          style={{
            ...
            borderRadius: "50%",
            flexShrink: 0,
            background: isDirty ? "#fbbf24" : "transparent",
