...
const sharedTextStyle: CSSProperties = {
  fontFamily: FONT, fontSize: FONT_SIZE, lineHeight: `${LINE_HEIGHT}px`,
  tabSize: 2, whiteSpace: "pre", wordWrap: "normal", overflowWrap: "normal",
  margin: 0, padding: `${PAD_TOP}px ${PAD_LEFT}px ${PAD_BOTTOM}px ${PAD_LEFT}px`,
...
export function LiveEditor({ code, lang, hljs, onChange, findMatches, activeMatch, style }: Props) {
  ...
  useEffect(() => { if (scrollRef.current) { scrollRef.current.scrollTop = 0; scrollRef.current.scrollLeft = 0; } }, [code.slice(0, 100)]);
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: "flex", overflow: "hidden", background: "#0b0e10", ...style }}>
      {/* Gutter */}
      <div style={{ width: gutterWidth, flexShrink: 0, overflow: "hidden", background: "#0b0e10", borderRight: `1px solid ${C.border}`, userSelect: "none", zIndex: 2 }}>
        <GutterInner lineCount={lineCount} lineHeight={LINE_HEIGHT} padTop={PAD_TOP} padBottom={PAD_BOTTOM} gutterWidth={gutterWidth} fontSize={FONT_SIZE} scrollRef={scrollRef} />
      </div>

      {/* Single scroll owner */}
      <div ref={scrollRef} className="sb-scroll" style={{ flex: 1, minWidth: 0, overflow: "auto", position: "relative" }}>
        <div style={{ position: "relative", width: contentW, height: contentH }}>
          {/* Highlight layer */}
          <div aria-hidden className="sb-hljs" style={{ ...sharedTextStyle, position: "absolute", top: 0, left: 0, width: "100%", minHeight: contentH, color: "#abb2bf", pointerEvents: "none", userSelect: "none", zIndex: 0 }}>
            {annotatedLines.map((html, i) => (
              <div key={i} style={{ height: LINE_HEIGHT, lineHeight: `${LINE_HEIGHT}px` }} dangerouslySetInnerHTML={{ __html: html || "\u00a0" }} />
            ))}
          </div>
          {/* Textarea */}
          <textarea
            ...
            onPaste={handlePaste}
            style={{ ...sharedTextStyle, position: "absolute", top: 0, left: 0, width: contentW, height: contentH, zIndex: 1, cursor: "text" }}
          />
        </div>
      </div>
    </div>
