      ...
      <div style={{
        ...
        <button onClick={onBack}
          style={{
            ...
            fontSize: 11, fontFamily: SANS, display: "flex", alignItems: "center", gap: 5,
            transition: "color .1s",
          }}
        ...
        <span style={{ fontSize: 12, fontFamily: MONO, fontWeight: 600, color: C.t0, padding: "0 10px", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {fileName}
        </span>

        <div style={{ display: "flex", gap: 6, paddingRight: 14, flexShrink: 0, fontSize: 11, fontFamily: MONO }}>
          {fc.insertions > 0 && <span style={{ color: "rgba(74,222,128,0.7)" }}>+{fc.insertions}</span>}
          {fc.deletions  > 0 && <span style={{ color: "rgba(248,113,113,0.7)" }}>-{fc.deletions}</span>}
        </div>
      </div>

      <div style={{
        padding: "0 14px", height: 26, flexShrink: 0,
          ...
          <table style={{ borderCollapse: "collapse", width: "100%", tableLayout: "fixed" }}>
            ...
            <tbody>
              {lines.map((line, i) => {
                ...
                const rowBg    = isAdd ? "rgba(74,222,128,0.07)" : isDel ? "rgba(248,113,113,0.07)" : isHunk ? "rgba(255,255,255,0.015)" : "transparent";
                const gutBg    = isAdd ? "rgba(74,222,128,0.12)" : isDel ? "rgba(248,113,113,0.12)" : "rgba(255,255,255,0.015)";
                const sigColor = isAdd ? "rgba(74,222,128,0.6)" : isDel ? "rgba(248,113,113,0.6)" : "transparent";
                const textColor = isAdd ? "rgba(180,230,180,0.85)" : isDel ? "rgba(230,170,170,0.85)" : isHunk ? "rgba(100,130,160,0.6)" : isMeta ? "rgba(100,100,100,0.5)" : "rgba(190,190,190,0.65)";

                return (
                  <tr key={i} style={{ background: rowBg }}>
                    <td style={{ padding: "0 8px", textAlign: "right", fontSize: 10, fontFamily: MONO, color: "rgba(100,100,100,0.45)", userSelect: "none", background: gutBg, borderRight: `1px solid ${C.border}`, lineHeight: "19px" }}>
                      {!isMeta && !isHunk ? i + 1 : ""}
                    </td>
                    <td style={{ textAlign: "center", fontFamily: MONO, fontSize: 11, color: sigColor, userSelect: "none", background: gutBg, borderRight: `1px solid ${C.border}`, lineHeight: "19px" }}>
                      {isAdd ? "+" : isDel ? "−" : ""}
                    </td>
                    <td style={{ paddingLeft: 10, paddingRight: 8, lineHeight: "19px" }}>
                      <span style={{ fontSize: 11.5, color: textColor, fontFamily: MONO, whiteSpace: "pre-wrap", wordBreak: "break-all", fontStyle: isHunk ? "italic" : "normal" }}>
                        {line || " "}
                      </span>
                    </td>
                  </tr>
...
export function FileChangeList({ runboxId, runboxCwd, onFileClick }: Props) {
  ...
  const handleFileClick = useCallback((fc: LiveDiffFile) => {
    setDiffFile(fc);
    onFileClick(fc);
  }, [onFileClick]);
      ...
      <div style={{
        ...
        {branch && (
          <>
            ...
            <span style={{ fontSize: 11, fontFamily: MONO, color: C.t1, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {branch}
            </span>
          </>
        )}
        ...
        {(totalIns > 0 || totalDel > 0) && (
          <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
            {totalIns > 0 && <span style={{ fontSize: 10, fontFamily: MONO, color: "rgba(74,222,128,0.65)", fontWeight: 600 }}>+{totalIns}</span>}
            {totalDel > 0 && <span style={{ fontSize: 10, fontFamily: MONO, color: "rgba(248,113,113,0.65)", fontWeight: 600 }}>-{totalDel}</span>}
          </div>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "4px 6px" }}>
        {loading && (
          <div style={{ padding: "32px 0", display: "flex", justifyContent: "center" }}>
            <div style={{ width: 14, height: 14, border: `2px solid ${C.border}`, borderTopColor: C.t1, borderRadius: "50%", animation: "spin .7s linear infinite" }} />
          </div>
        )}
        {!loading && error && (
          <div style={{ margin: "6px", padding: "10px 12px", background: "rgba(248,113,113,0.06)", border: `1px solid rgba(248,113,113,0.15)`, borderRadius: 8, fontSize: 12, color: "rgba(248,113,113,0.8)", fontFamily: SANS }}>
            {error}
            <button onClick={() => reload(false)} style={{ display: "block", marginTop: 8, padding: "4px 10px", background: C.bg3, border: `1px solid ${C.border}`, borderRadius: 6, color: C.t2, fontSize: 11, fontFamily: SANS, cursor: "pointer" }}>
              Retry
            </button>
          </div>
        )}
        {!loading && !error && files.length === 0 && (
          <div style={{ padding: "40px 0", display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={C.t3} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round">
        ...
        {!loading && !error && files.map(fc => {
          ...
          return (
            <div key={fc.path} onClick={() => handleFileClick(fc)}
              style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  <span style={{ fontSize: 12, fontFamily: MONO, color: C.t0, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {fileName}
                  </span>
                  <span style={{ fontSize: 9, fontFamily: MONO, fontWeight: 700, color: st.color, flexShrink: 0 }}>{letter}</span>
                </div>
              ...
              {agentName && <span style={{ fontSize: 9, fontFamily: MONO, color: C.t3, background: C.bg3, borderRadius: 4, padding: "1px 5px", flexShrink: 0 }}>{agentName}</span>}

              <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                {fc.insertions > 0 && <span style={{ fontSize: 10, fontFamily: MONO, color: "rgba(74,222,128,0.6)", fontWeight: 600 }}>+{fc.insertions}</span>}
                {fc.deletions  > 0 && <span style={{ fontSize: 10, fontFamily: MONO, color: "rgba(248,113,113,0.6)", fontWeight: 600 }}>-{fc.deletions}</span>}
              </div>
