...
export function AddLockedForm({ workspaceId, onAdded }: Props) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      await invoke("memory_add_locked", { runboxId: workspaceId, sessionId: `panel-${workspaceId}`, content: content.trim() });
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)}
      style={{ width: "100%", padding: "9px 14px", borderRadius: 9, background: "transparent", border: `1px dashed ${C.amber}4d`, color: C.amber, fontSize: 11, fontFamily: SANS, cursor: "pointer", transition: "all .15s", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
    ...
    <div style={{ background: C.amberBg, border: `1px solid ${C.amber}40`, borderRadius: 11, padding: 13, display: "flex", flexDirection: "column", gap: 9 }}>
      ...
      <div style={{ display: "flex", gap: 7 }}>
        ...
        <button onClick={submit} disabled={loading || !content.trim()}
          style={{ flex: 1, padding: "7px 0", borderRadius: 8, border: "none", background: content.trim() && !loading ? C.amber : C.bg4, color: content.trim() && !loading ? C.bg0 : C.t2, fontSize: 12, fontWeight: 600, fontFamily: SANS, cursor: content.trim() && !loading ? "pointer" : "default", transition: "all .15s" }}>
