...
export function ContextPreview({ workspaceId }: Props) {
  ...
  const load = useCallback(async (t: string) => {
    setLoading(true);
    try { setContext(await invoke<string>("memory_get_context", { runboxId: workspaceId, task: t || null }) || "No context yet."); }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, height: "100%", padding: "8px 10px 0" }}>
      ...
      <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
        ...
        <button onClick={() => load(task)} disabled={loading}
          style={{ padding: "7px 12px", borderRadius: 8, border: `1px solid ${C.border}`, background: C.bg3, color: loading ? C.t2 : C.t0, fontSize: 11, fontFamily: SANS, cursor: loading ? "default" : "pointer" }}>
          {loading ? "…" : "↺"}
        </button>
      </div>

      <div style={{ flex: 1, overflow: "auto", paddingBottom: 16 }}>
        <pre style={{ margin: 0, fontSize: 11.5, fontFamily: MONO, color: C.t1, lineHeight: 1.7, whiteSpace: "pre-wrap", wordBreak: "break-word", background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 10, padding: "12px 14px" }}>
          {context}
        </pre>
      </div>
    </div>
