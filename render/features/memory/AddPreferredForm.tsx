...
export function AddPreferredForm({ workspaceId, onAdded }: Props) {
  ...
  return (
    ...
    >
      <div style={{ fontSize: 10, fontFamily: MONO, color: C.blue, letterSpacing: ".06em" }}>
        ◎ NEW PREFERRED FACT
      </div>
      <div style={{ fontSize: 10, color: C.t3, fontFamily: MONO }}>
        Key=value for env: port=3456, node=v18. One atomic fact per save.
      </div>
      <textarea
        ref={taRef}
        value={content}
      ...
      <div style={{ display: "flex", gap: 7 }}>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 8,
            border: "none",
            background: content.trim() && !loading ? C.blue : C.bg4,
            color: content.trim() && !loading ? C.tealBright : C.t2,
            fontSize: 12,
            fontWeight: 600,
            fontFamily: SANS,
