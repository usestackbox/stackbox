...
import type { Memory, MemTab } from "./memoryTypes";
...
export function useMemory(workspaceId: string) {
  ...
  useEffect(() => {
    ...
    const errorUnsub = listen<string>("memory-error", ({ payload }) => {
      if (!cancelled) { setError(`Memory error: ${payload}`); setLoading(false); }
    });
      ...
      while (!cancelled) {
        try {
          ...
          if (msg.includes("not initialised") && attempt < DELAYS.length) {
            ...
            if (!cancelled) { setError("Memory took too long. Click Retry."); setLoading(false); }
            return;
          }
        }
      }
  ...
  const retry = useCallback(() => {
    setError(null); setDbReady(false); setLoading(true);
    setRetryKey(k => k + 1);
  }, []);
