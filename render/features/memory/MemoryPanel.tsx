...
export function MemoryPanel({ workspaceId, workspaceName, onClose }: Props) {
  const {
    memories, loading, error, dbReady,
    locked, preferred, temporary, session,
    handleDelete, handlePin, handleEdit, loadAll, retry,
  ...
  const visible = tab === "context" ? [] : (tabMap[tab] ?? []).filter(m =>
  ...
  if (!dbReady) return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: C.bg1, alignItems: "center", justifyContent: "center", gap: 12, padding: "0 24px" }}>
      {!error
        ...
        : (<><span style={{ fontSize: 11, color: C.t2, fontFamily: SANS, textAlign: "center", lineHeight: 1.6 }}>{error}</span><button onClick={retry} style={{ padding: "6px 14px", borderRadius: 8, border: `1px solid ${C.border}`, background: C.bg2, color: C.t1, fontSize: 11, fontFamily: SANS, cursor: "pointer" }}>Retry</button></>)
      }
      <style>{SPIN}</style>
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: C.bg1 }}>
      ...
      <div style={{ padding: "12px 14px 11px", flexShrink: 0, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: C.t0, flex: 1, fontFamily: SANS, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{workspaceName}</span>
      ...
      {tab === "context" ? (
        ...
        <>
          ...
          <div style={{ padding: "8px 10px", borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
            <div style={{ position: "relative" }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…"
                style={{ width: "100%", boxSizing: "border-box", background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 8, color: C.t0, fontSize: 11, padding: "7px 28px 7px 28px", outline: "none", fontFamily: MONO }} />
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "10px 10px 16px", display: "flex", flexDirection: "column", gap: 7 }}>
            {tab === "LOCKED"    && <AddLockedForm    workspaceId={workspaceId} onAdded={() => { loadAll(); showToast("🔒 Locked rule added"); }} />}
            {tab === "PREFERRED" && <AddPreferredForm workspaceId={workspaceId} onAdded={() => { loadAll(); showToast("◎ Fact saved"); }} />}
