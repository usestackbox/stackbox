  ...
  return createPortal(
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 9999 }}>
      <div onClick={e => e.stopPropagation()} style={{
        ...
        background: "rgba(18,20,24,0.96)", backdropFilter: "blur(20px)",
        border: "1px solid rgba(255,255,255,.08)", borderRadius: 9, padding: 4,
        boxShadow: "0 24px 64px rgba(0,0,0,.75)", minWidth: 170, zIndex: 99999,
      }}>
        <div style={{ padding: "3px 10px 5px", fontSize: 9, fontFamily: MONO, color: "rgba(255,255,255,.22)", letterSpacing: ".1em" }}>
        ...
        {[
          ...
          { label: "Delete",    danger: true, action: () => { onDelete(); onClose(); } },
        ].map(item => (
          <div key={item.label} onClick={item.action}
            style={{ display: "flex", alignItems: "center", padding: "0 10px", height: 28, borderRadius: 6, cursor: "pointer", color: (item as any).danger ? "#f87171" : "rgba(255,255,255,.78)", fontSize: 13, fontFamily: SANS, fontWeight: 400, transition: "background .08s" }}
            onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = (item as any).danger ? "rgba(239,68,68,.18)" : "rgba(255,255,255,.08)"}
            onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = "transparent"}>
            {item.label}
          </div>
        ))}
...
function RenameModal({ entry, onConfirm, onCancel }: { entry: FsEntry; onConfirm: (name: string) => void; onCancel: () => void }) {
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 9998, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,.5)" }}
      ...
      <div style={{ background: "#252526", border: "1px solid #454545", borderRadius: 6, padding: "16px 20px", minWidth: 320, display: "flex", flexDirection: "column", gap: 12, boxShadow: "0 8px 32px rgba(0,0,0,.6)" }}
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button onClick={onCancel} style={{ background: "transparent", border: "1px solid #454545", borderRadius: 3, color: "#cccccc", fontSize: 12, padding: "5px 16px", cursor: "pointer" }}>Cancel</button>
          <button onClick={() => onConfirm(value.trim())} style={{ background: "#0e639c", border: "none", borderRadius: 3, color: "#fff", fontSize: 12, padding: "5px 16px", cursor: "pointer" }}>Rename</button>
        </div>
      </div>
    </div>,
    document.body,
...
export function FilePanel({ cwd, onClose, onOpenFile }: Props) {
  ...
  const getActiveDir = (): string => {
    ...
    const parts = selected.replace(/\\/g, "/").split("/"); parts.pop();
  ...
  const handleRename = async (newName: string) => {
    ...
    try { await invoke("fs_rename", { from: renaming.path, to: parts.join(sep) }); setRefreshKey(k => k + 1); }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: C.bg1 }}
      ...
      <div style={{
        ...
        padding: "0 8px 0 14px", borderBottom: `1px solid ${C.border}`, gap: 4,
      }}>
        <span style={{
          flex: 1, fontSize: 13, letterSpacing: ".06em",
          fontFamily: MONO, fontWeight: 400,
          color: "rgba(255,255,255,.55)", userSelect: "none",
        }}>
          FILES
        </span>
