...
import { FileIcon, fileColor } from "./FileIcon";
import { CreateInput } from "./CreateInput";
import { DeleteModal } from "./DeleteModal";

interface FsEntry { name: string; path: string; is_dir: boolean; ext?: string; }
...
function CtxMenu({ menu, onClose, items }: { menu: CtxMenuState; onClose: () => void; items: CtxMenuItem[] }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => { document.addEventListener("mousedown", onDown); document.addEventListener("keydown", onKey); }, 50);
  ...
  return createPortal(
    <div ref={ref} onClick={e => e.stopPropagation()} style={{
      position: "fixed", top: y, left: x, zIndex: 99999, minWidth: 175,
      background: "rgba(18,20,24,0.96)", backdropFilter: "blur(28px)",
      border: "1px solid rgba(255,255,255,0.08)", borderRadius: 9, padding: 4,
...
function CtxItem({ label, shortcut, danger, onClick, onClose }: { label: string; shortcut?: string; danger?: boolean; onClick: () => void; onClose: () => void }) {
  const [hov, setHov] = useState(false);
  return (
    <div onClick={() => { onClick(); onClose(); }}
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{
        display: "flex", alignItems: "center", padding: "0 10px", height: 28, borderRadius: 6, cursor: "pointer",
...
function TreeRow({ entry, depth, expanded, selected, onToggle, onSelect, onOpen, onContextMenu }: {
  ...
  const color = entry.is_dir
    ? (depth === 0 ? "rgba(120,200,200,.9)" : "rgba(120,200,200,.65)")
    : "rgba(180,185,195,.75)";
  ...
  return (
    <div
      ...
      <span style={{ width: 12, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
        {entry.is_dir && (
          <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(150,160,170,.7)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
...
// ── TreeNode ──────────────────────────────────────────────────────────────────
function TreeNode({ path, name, depth, selectedPath, onSelect, onOpen, creatingIn, onCreatingDone, onCreatingCancel, onContextMenu, refreshKey }: {
  ...
  useEffect(() => { if (refreshKey && open) load(); }, [refreshKey]); // eslint-disable-line
  useEffect(() => { if (creatingIn?.parentPath === path && !open) { setOpen(true); if (children === null) load(); } }, [creatingIn?.parentPath]); // eslint-disable-line
  useEffect(() => { if (open && children === null) load(); }, [open]); // eslint-disable-line
        ...
        <>
          ...
          {children?.map(child =>
            child.is_dir
              ? <TreeNode key={child.path} path={child.path} name={child.name} depth={depth + 1}
                  ...
                  onContextMenu={onContextMenu} refreshKey={refreshKey} />
          ...
          {!loading && children?.length === 0 && !showCreate && (
            <div style={{ paddingLeft: 8 + (depth + 1) * 14 + 28, height: 22, display: "flex", alignItems: "center" }}>
              <span style={{ fontSize: 12, color: "rgba(255,255,255,.2)", fontFamily: SANS, fontStyle: "italic" }}>empty</span>
            </div>
          )}
        </>
      )}
...
// ── FileTree (exported) ───────────────────────────────────────────────────────
export function FileTree({ cwd, selectedPath, refreshKey, creating, onSelect, onOpen, onRefresh, onCreatingDone, onCreatingCancel, onContextMenu }: {
  cwd: string; selectedPath: string | null; refreshKey: number;
  ...
  const handleCtx = useCallback((e: React.MouseEvent, entry: FsEntry) => {
    onContextMenu?.(e, entry);
  }, [onContextMenu]);

  return (
    <>
      <TreeNode key={cwd} path={cwd} name={rootName} depth={0}
