...
export function DeleteModal({ entry, onConfirm, onCancel }: Props) {
  ...
  return createPortal(
    <div
      ...
      onClick={onCancel}>
      <div
        style={{ background: "#252526", border: "1px solid #454545", borderRadius: 8, padding: "20px 24px", minWidth: 340, maxWidth: 440, display: "flex", flexDirection: "column", gap: 14, boxShadow: "0 12px 40px rgba(0,0,0,.7)" }}
        ...
        <div style={{ fontSize: 13, color: "#999", lineHeight: 1.6 }}>
          ...
          <span style={{ color: "#e6edf3", fontFamily: MONO, fontWeight: 500 }}>"{entry.name}"</span>?
          {entry.is_dir && (
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 2 }}>
          ...
          <button ref={btnRef} onClick={onConfirm}
            ...
            onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = "rgba(239,68,68,.18)"}>
            Delete
          </button>
        </div>
      </div>
