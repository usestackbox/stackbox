...
export function NoGitPane({ cwd, onClose, onInitDone }: Props) {
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText("git init");
    } catch {
      /* */
    }
    setCopied(true);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: C.bg1 }}>
      ...
      >
        ...
        >
          Run this in the terminal — the panel will
          <br />
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              border: `1px solid ${copied ? C.green : C.borderMd}`,
              borderRadius: 6,
              color: copied ? C.green : C.t1,
              fontSize: 10,
              fontFamily: SANS,
              cursor: "pointer",
              transition: "all .15s",
            ...
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div
                style={{
                  ...
                  height: 5,
                  borderRadius: "50%",
                  background: i === 3 ? C.green : C.t3,
                  flexShrink: 0,
                }}
              />
              <span style={{ fontSize: 11, fontFamily: SANS, color: i === 3 ? C.green : C.t2 }}>
                {step}
              </span>
            </div>
          ))}
