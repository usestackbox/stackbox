...
export function WorktreesTab({ worktrees, onCreateWorktree, onDiff }: Props) {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await onCreateWorktree(wtName.trim(), newBranch.trim());
      setShowNewWt(false);
      setWtName("");
      setNewBranch("");
    } finally {
  ...
  const handleDiff = async (wtPath: string) => {
    if (diffTarget === wtPath) {
      setDiffTarget(null);
      setDiffResult("");
      return;
    }
    setDiffTarget(wtPath);
    setDiffLoading(true);
    try {
      setDiffResult((await onDiff(wtPath)) || "No differences.");
    } catch (e: any) {
      setDiffResult(String(e));
    } finally {
      ...
      {!showNewWt ? (
        ...
        >
          ...
          <input
            ...
            placeholder={`Branch (default: ${wtName || "name"})`}
            style={{
              background: C.bg0,
              border: `1px solid ${C.border}`,
              borderRadius: 8,
          ...
          <div style={{ display: "flex", gap: 6 }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border: "none",
                background: wtName.trim() && !creating ? C.t0 : C.bg4,
                color: wtName.trim() && !creating ? C.bg0 : C.t3,
                fontSize: 11,
                fontFamily: SANS,
                fontWeight: 600,
                cursor: wtName.trim() && !creating ? "pointer" : "default",
              }}
            >
            ...
            <div style={{ padding: "9px 10px", display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                  <span
                    style={{
                      fontSize: 12,
                      fontFamily: MONO,
                      color: wt.is_main ? C.t0 : C.t1,
                      fontWeight: wt.is_main ? 500 : 400,
                      overflow: "hidden",
                  ...
                  {wt.is_locked && (
                    <span style={{ fontSize: 9, color: C.t2, fontFamily: SANS, flexShrink: 0 }}>
                      locked
                    </span>
                  )}
                </div>
              ...
              {!wt.is_main && (
                <button
                  ...
                  style={{
                    padding: "4px 9px",
                    background: isDiffOpen ? C.bg4 : "transparent",
                    border: `1px solid ${isDiffOpen ? C.borderMd : C.border}`,
                    borderRadius: 6,
                    color: isDiffOpen ? C.t0 : C.t2,
                    fontSize: 10,
                    fontFamily: SANS,
                    cursor: "pointer",
                    flexShrink: 0,
