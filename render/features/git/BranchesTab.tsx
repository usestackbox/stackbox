...
function AgentBranchRow({
  ab, onMerge, onDelete, onBranchLog, onBranchStatus,
}: {
  ab:              AgentBranch;
  ...
  const handleMerge = async () => {
    setMerging(true);
    try { await onMerge(ab.branch); }
    finally { setMerging(false); }
  };

  const handleDelete = async (force = false) => {
    setDeleting(true);
    try { await onDelete(ab.branch, force); }
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "9px 10px", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 2 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ fontSize: 9, fontFamily: MONO, color: C.t3, background: C.bg4, borderRadius: 4, padding: "1px 5px", flexShrink: 0 }}>
        ...
        <div style={{
          ...
          {commits.map(c => (
            <div key={c.hash} style={{
              ...
              <span style={{ fontSize: 10, fontFamily: MONO, color: C.t3, flexShrink: 0, marginTop: 1 }}>
                {c.short_hash}
              </span>
      ...
      {expanded && commits.length === 0 && (
        <div style={{ padding: "8px 12px", fontSize: 11, color: C.t3, fontFamily: SANS, borderTop: `1px solid ${C.border}` }}>
  ...
  return (
    <button
      ...
      style={{
        ...
        fontSize: 10, fontFamily: SANS, cursor: disabled || loading ? "default" : "pointer",
        transition: "all .1s", fontWeight: primary ? 600 : 400,
        opacity: disabled ? 0.5 : 1,
      }}
      onMouseEnter={e => {
        if (!disabled && !loading) {
          const el = e.currentTarget as HTMLElement;
          if (!primary) { el.style.borderColor = danger ? "#f87171" : C.borderMd; el.style.color = danger ? "#f87171" : C.t0; }
        }
      }}
      onMouseLeave={e => {
        const el = e.currentTarget as HTMLElement;
        if (!primary) { el.style.borderColor = danger ? "#f8717166" : C.border; el.style.color = color; }
      }}
    >
      {loading ? "…" : label}
    </button>
...
export function BranchesTab({
  ...
  onMerge, onDelete, onBranchLog, onBranchStatus,
}: Props) {
  ...
  const BranchRow = ({ b }: { b: string }) => {
    ...
    return (
      <div style={{
        ...
        display: "flex", flexDirection: "column", gap: isRenamingThis ? 8 : 0,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{
            width: 6, height: 6, borderRadius: "2px",
            background: isActive ? C.t0 : C.t3, flexShrink: 0,
          }} />
          <span style={{
          ...
          {isRemote && !isActive && (
            <span style={{ fontSize: 9, fontFamily: MONO, color: C.t3, background: C.bg4, borderRadius: 4, padding: "1px 5px" }}>remote</span>
          )}
          {isActive && (
            <span style={{ fontSize: 10, fontFamily: SANS, color: C.t3, background: C.bg4, borderRadius: 6, padding: "2px 7px" }}>current</span>
          )}
          {isActive && !isRemote && onRename && !isRenamingThis && (
            <button onClick={() => { setRenamingId(clean); setRenameVal(clean); }}
              style={{ padding: "4px 10px", background: "transparent", border: `1px solid ${C.border}`, borderRadius: 6, color: C.t2, fontSize: 10, fontFamily: SANS, cursor: "pointer" }}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <button onClick={() => { setRenamingId(null); setRenameVal(""); }}
                style={{ padding: "6px 12px", background: "transparent", border: `1px solid ${C.border}`, borderRadius: 7, color: C.t2, fontSize: 11, fontFamily: SANS, cursor: "pointer" }}>
                Cancel
              </button>
              <button onClick={handleRename} disabled={renaming || !renameVal.trim() || renameVal.trim() === renamingId}
                style={{
                  ...
                  background: renameVal.trim() && renameVal.trim() !== renamingId && !renaming ? C.t0 : C.bg4,
                  color: renameVal.trim() && renameVal.trim() !== renamingId && !renaming ? C.bg0 : C.t3,
                  fontSize: 11, fontFamily: SANS, fontWeight: 600, cursor: "pointer",
                }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, overflowY: "auto" }}>
      ...
      <div style={{ flex: 1, padding: "0 8px 8px", display: "flex", flexDirection: "column", gap: 3 }}>
        {filtered.length === 0 && allBranches.filter(b => !b.startsWith("stackbox/")).length === 0 && (
          <div style={{ padding: "16px 0", textAlign: "center", fontSize: 12, color: C.t2, fontFamily: SANS }}>No branches found.</div>
        )}
