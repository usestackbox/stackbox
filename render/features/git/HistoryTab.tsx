...
function InlineDiff({ diff }: { diff: string }) {
  ...
  return (
    <div style={{
      maxHeight: 360, overflowY: "auto", fontSize: 11, fontFamily: MONO,
      ...
      {diff.split("\n").map((line, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            background: isAdd ? "rgba(74,222,128,.06)" : isDel ? "rgba(248,113,113,.06)" : "transparent",
      ...
      {commits.map((c, i) => {
        ...
        return (
          <div key={c.hash} style={{
            ...
            borderRadius: 8, overflow: "hidden", transition: "all .1s",
          }}>
            {/* Commit row */}
            <div
              onClick={() => handleToggle(c.hash)}
              style={{ padding: "9px 10px", cursor: "pointer", display: "flex", gap: 10, alignItems: "flex-start" }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = C.bg4; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = ""; }}>

              {/* Graph dot */}
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0, paddingTop: 3, gap: 3 }}>
                <div style={{
                ...
                {i < commits.length - 1 && <div style={{ width: 1, height: 16, background: C.border }} />}
              </div>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  {isFirst && (
                    <span style={{ fontSize: 9, fontFamily: MONO, color: C.blue, background: C.blueBg, border: `1px solid ${C.blue}30`, borderRadius: 4, padding: "1px 5px" }}>
                      HEAD
                    </span>
                  )}
                </div>
            ...
            {isExpanded && loading && (
              <div style={{ padding: "12px 12px", display: "flex", alignItems: "center", gap: 8, borderTop: `1px solid ${C.border}` }}>
                <div style={{ width: 10, height: 10, border: `1.5px solid ${C.border}`, borderTopColor: C.t1, borderRadius: "50%", animation: "htspin .7s linear infinite" }} />
