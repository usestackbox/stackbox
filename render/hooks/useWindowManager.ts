...
export function useWindowManager(initialCwd: string, areaRef: React.RefObject<HTMLDivElement>) {
  ...
  const initFirstWindow = useCallback(() => {
    ...
    setWins([{ id, label: "w1", kind: "terminal", x: GAP, y: GAP, w: aw - GAP * 2, h: ah - GAP * 2, minimized: false, maximized: false, cwd: initialCwd, zIndex: nextZ() }]);
    setActiveWinId(id);
  }, [initialCwd, areaRef]);
  ...
  const addTerminal = useCallback((cwd: string) => {
    ...
    const id = crypto.randomUUID();
    setWins(prev => {
      const all: WinState[] = [...prev, { id, label: `w${labelCount.current}`, kind: "terminal", x: 0, y: 0, w: 400, h: 300, minimized: false, maximized: false, cwd, zIndex: nextZ() }];
  ...
  const handleResizeStart = useCallback((e: React.MouseEvent, id: string, dir: string) => {
    ...
    const win = winsRef.current.find(w => w.id === id);
    if (!win || win.maximized) return;
    const { x: startX, y: startY, x: origX, y: origY, w: origW, h: origH } = { ...win, x: e.clientX, y: e.clientY };
  ...
  return {
    wins, setWins, activeWinId, setActiveWinId,
    focusWin, initFirstWindow, addTerminal, closeWin,
    minimizeWin, restoreWin, maximizeWin, moveTab,
    handleDragStart, handleResizeStart, retile,
  };
}
