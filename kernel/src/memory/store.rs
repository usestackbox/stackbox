...
use super::schema::{
    agent_type_from_name, decay_for_level, decay_for_type, dedup_threshold, extract_key,
    importance_for_level, importance_for_type, infer_type_from_tags, level_from_memory_type,
    make_agent_id, memory_schema, now_ms, null_vector, Memory, DECAY_NEVER, DECAY_SESSION,
    EMBEDDING_DIM, LEVEL_LOCKED, LEVEL_PREFERRED, LEVEL_SESSION, LEVEL_TEMPORARY, MT_BLOCKER,
    MT_ENVIRONMENT, MT_FAILURE, SCOPE_LOCAL, SCOPE_MACHINE,
};
...
pub async fn init() -> Result<(), String> {
    DB.get_or_try_init(|| async {
        ...
        if tables.contains(&"memories".to_string()) {
            ...
            if wrong_dim {
                eprintln!(
                    "[memory] vector dim mismatch — recreating with dim={}",
                    EMBEDDING_DIM
                );
...
async fn migrate_v2_to_v3(conn: &Connection, backup: &str) -> Result<(), String> {
    ...
    for batch in &batches {
        ...
        // Derive V3 fields
        let levels: Vec<String> = mem_types
            .iter()
            .map(|mt| level_from_memory_type(mt).to_string())
            .collect();
        let agent_ids: Vec<String> = (0..n)
            .map(|i| make_agent_id(&agent_types[i], &sess_ids[i]))
            .collect();
        let keys: Vec<String> = contents.iter().map(|c| extract_key(c)).collect();

        // Drop git-type memories (noise)
        let keep: Vec<usize> = (0..n).filter(|&i| mem_types[i] != "git").collect();
        if keep.is_empty() {
            continue;
        }
        ...
        let idx_filter_bool = |v: Vec<bool>| -> Vec<bool> { keep.iter().map(|&i| v[i]).collect() };
        let idx_filter_i64 = |v: Vec<i64>| -> Vec<i64> { keep.iter().map(|&i| v[i]).collect() };
        let idx_filter_i32 = |v: Vec<i32>| -> Vec<i32> { keep.iter().map(|&i| v[i]).collect() };
        ...
        let new_batch = RecordBatch::try_new(
            ...
            vec![
                Arc::new(StringArray::from(
                    ids2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    runbox_ids2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    sess_ids2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    contents2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(BooleanArray::from(pinneds2)),
                Arc::new(Int64Array::from(timestamps2)),
                Arc::new(StringArray::from(
                    branches2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    commit_types2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    tags_vec2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    parent_ids2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    agent_names2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    mem_types2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(Int32Array::from(importances2)),
                Arc::new(BooleanArray::from(resolveds2)),
                Arc::new(Int64Array::from(decay_ats2)),
                Arc::new(StringArray::from(
                    scopes2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    agent_types2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    levels2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    agent_ids2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                Arc::new(StringArray::from(
                    keys2.iter().map(|s| s.as_str()).collect::<Vec<_>>(),
                )),
                null_vector_n(n2)?,
            ],
        )
...
fn get_conn() -> Result<&'static Connection, String> {
    DB.get()
        .ok_or_else(|| "memory db not initialised".to_string())
}
...
fn str_col<'a>(batch: &'a RecordBatch, name: &str) -> Result<&'a StringArray, String> {
    let idx = batch
        .schema()
        .index_of(name)
        .map_err(|_| format!("col '{}' missing", name))?;
    batch
        .column(idx)
        .as_any()
...
fn batch_to_memory(batch: &RecordBatch, i: usize) -> Result<Memory, String> {
    ...
    let memory_type = if mt_raw.is_empty() {
        infer_type_from_tags(&tags).to_string()
    } else {
        mt_raw
    };
    let imp_raw = i32_or("importance", 0);
    let importance = if imp_raw == 0 {
        importance_for_type(&memory_type)
    } else {
        imp_raw
    };
    let agent_type = {
        ...
        if at.is_empty() {
            agent_type_from_name(&agent_name)
        } else {
            at
        }
    };
...
pub async fn remember(
    ...
    if level == LEVEL_PREFERRED && !key.is_empty() {
        ...
        for old in existing
            .iter()
            .filter(|m| m.effective_level() == LEVEL_PREFERRED && m.key == key && !m.resolved)
        {
...
pub async fn session_log(
    ...
    let existing = memories_for_runbox(runbox_id).await.unwrap_or_default();
    let mut agent_logs: Vec<_> = existing
        .iter()
        .filter(|m| {
...
pub async fn expire_temporary_for_agent(runbox_id: &str, agent_id: &str) -> Result<(), String> {
    let all = memories_for_runbox(runbox_id).await.unwrap_or_default();
    let to_expire: Vec<String> = all
        .iter()
        .filter(|m| m.agent_id == agent_id && m.effective_level() == LEVEL_TEMPORARY && !m.resolved)
        .map(|m| m.id.clone())
        .collect();
...
pub async fn memory_add_typed(
    ...
    if threshold <= 1.0 {
        if let Some(ref emb) = embedding_opt {
            if let Ok(Some(dup)) =
                find_semantic_duplicate(runbox_id, memory_type, emb, threshold).await
            {
...
pub async fn memory_add_with_embedding(
    ...
    Ok(Memory {
        ...
        timestamp: ts,
        branch: "main".into(),
        commit_type: "memory".into(),
        tags: "".into(),
        parent_id: "".into(),
        agent_name: "".into(),
        memory_type: MT_ENVIRONMENT.into(),
        importance: 90,
        resolved: false,
        decay_at: decay_for_level(LEVEL_PREFERRED),
        scope: SCOPE_LOCAL.into(),
        agent_type: "".into(),
        level: LEVEL_PREFERRED.into(),
        agent_id: "".into(),
        key: "".into(),
    })
}
...
pub async fn memories_ann_search(
    ...
    let mut all = memories_for_runbox(if runbox_id.is_empty() {
        "__all__"
    } else {
        runbox_id
    })
    ...
    let mut results: Vec<(f32, Memory)> = all
        .into_iter()
        .filter(|m| {
            if let Some(tf) = memory_type {
                if m.effective_type() != tf {
                    return false;
                }
            }
            format!("{} {} {}", m.content, m.tags, m.memory_type)
                .to_lowercase()
                .contains(&ql)
        })
...
async fn query_raw(filter: &str, limit: usize) -> Result<Vec<Memory>, String> {
    ...
    out.sort_by(|a, b| {
        if a.pinned != b.pinned {
            return a.pinned.cmp(&b.pinned).reverse();
        }
        b.importance
            .cmp(&a.importance)
...
pub async fn memories_by_tag(runbox_id: &str, tag: &str) -> Result<Vec<Memory>, String> {
    ...
    Ok(all
        .into_iter()
        .filter(|m| m.tags.split(',').any(|t| t.trim().to_lowercase() == tl))
        .collect())
}
...
pub async fn memories_by_type(runbox_id: &str, memory_type: &str) -> Result<Vec<Memory>, String> {
    ...
    Ok(all
        .into_iter()
        .filter(|m| m.effective_type() == memory_type)
        .collect())
}
...
pub async fn memories_by_level(runbox_id: &str, level: &str) -> Result<Vec<Memory>, String> {
    ...
    Ok(all
        .into_iter()
        .filter(|m| m.effective_level() == level)
        .collect())
}
...
pub async fn resolve_blocker(
    ...
    let blocker = all.into_iter().find(|m| {
        m.effective_type() == MT_BLOCKER
            && !m.resolved
...
pub async fn memories_delete_for_runbox(runbox_id: &str) -> Result<(), String> {
    ...
    let all = memories_for_runbox(runbox_id).await?;
    let ids_to_delete: Vec<String> = all
        .iter()
...
pub async fn memory_move_branch(id: &str, branch: &str) -> Result<(), String> {
    ...
    crate::memory::decision::assert_not_locked(&mem)?;
    let mut updated = mem.clone();
    updated.branch = branch.to_string();
    delete_and_reinsert(&mem.id, updated).await
}
...
pub async fn memories_search_global(query: &str, limit: usize) -> Result<Vec<Memory>, String> {
    ...
    for batch in &batches {
        ...
        for i in 0..n {
            let mem = batch_to_memory(batch, i)?;
            if mem.agent_name == "git" && query.len() < 4 {
                continue;
            }
