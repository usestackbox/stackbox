...
pub fn level_from_memory_type(mt: &str) -> &'static str {
    match mt {
        ...
        MT_FAILURE | MT_ENVIRONMENT | MT_CODEBASE | MT_GENERAL => LEVEL_PREFERRED,
...
pub fn infer_type_from_tags(tags: &str) -> &'static str {
    ...
    if t.contains("codebase") {
        return MT_CODEBASE;
    }
    if t.contains("git-history") {
        return MT_GIT;
    }
    MT_GENERAL
}
...
impl Memory {
    ...
    pub fn env_unverified(&self) -> bool {
        self.effective_type() == MT_ENVIRONMENT && (now_ms() - self.timestamp) > 30 * 86_400_000
    }
