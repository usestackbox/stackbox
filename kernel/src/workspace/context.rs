...
| `.stackbox/log/{branch_file}.md` | Your full execution trace — **append, never delete** |
| `.stackbox/meta/{branch_file}.yaml` | Stack, files, env keys, port — **add notes here** |
...
- Stay inside `{cwd}` — do not `cd` above this
- Do not run `git checkout`, `git worktree`, or `git switch`
- Your branch is `{branch}` — never push to main directly
...
git fetch origin
git diff {branch}..stackbox/<their-id>
```
...
session_log(entry="[done] fixed, tests passing at localhost:{pane_port}")
session_log(entry="[error] port 3000 occupied — using {pane_port}")
session_log(entry="[blocked] waiting on DB migration from agent-2")
```
Also append the same line to `.stackbox/log/{branch_file}.md`.
...
```
session_summary(text="Fixed JWT refresh race in api/auth.ts. Node v20, port {pane_port}.
Did not touch frontend. Branch {branch} — PR opened at <url>.
...
| Your history | `cat .stackbox/log/{branch_file}.md` |
...
- `session_log` after each step — also append to `.stackbox/log/{branch_file}.md`
...
- Stay inside `{cwd}` — never navigate above it
- Never push to `main` directly — branch is `{branch}`
...
pub async fn build(
    ...
    let memory_block = if *agent != AgentKind::Shell {
        ...
        if block.trim().is_empty() { String::new() }
        else { format!("## Memory\n\n{block}\n") }
    } else {
        String::new()
    };
