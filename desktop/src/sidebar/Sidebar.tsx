...
import { NotificationBell, NotificationPanel } from "../shared/Notificationsystem";
...
function GitBadge({ stats }: { stats: GitStats }) {
  ...
  return (
    <div style={{
      ...
      {stats.insertions > 0 && (
        <span style={{ fontSize: 11, fontFamily: MONO, color: "#4ade80", fontWeight: 600 }}>
          +{stats.insertions}
        </span>
      )}
      {stats.deletions > 0 && (
        <span style={{ fontSize: 11, fontFamily: MONO, color: "#f87171", fontWeight: 600 }}>
          -{stats.deletions}
        </span>
      )}
    </div>
...
function IconPicker({
  ...
  const filteredIcons = search.trim()
    ? ICON_GROUPS.flatMap(g => g.icons).filter(ic =>
        ic.includes(search) ||
        ic.codePointAt(0)?.toString(16).includes(search.toLowerCase())
      )
      ...
      <div style={{ padding: "10px 10px 6px", flexShrink: 0 }}>
        <div style={{
          ...
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
            stroke="rgba(255,255,255,.35)" strokeWidth="2.5" strokeLinecap="round">
          ...
          {ICON_GROUPS.map((g, i) => (
            <button
              ...
              style={{
                ...
                padding: "3px 9px", fontSize: 10, fontFamily: MONO, fontWeight: 700,
                letterSpacing: ".08em", whiteSpace: "nowrap",
                background: activeGroup === i ? "rgba(124,109,250,.25)" : "transparent",
                color: activeGroup === i ? "#a89dff" : "rgba(255,255,255,.35)",
                transition: "all .12s",
              }}
            >{g.label}</button>
          ))}
...
function ContextMenu({
  x, y, rbId, rbName,
  onDelete, onChangeIcon, onClose,
}: {
  x: number; y: number;
  rbId: string; rbName: string;
  onDelete: () => void;
  onChangeIcon: () => void;
  onClose: () => void;
}) {
  ...
  return (
    ...
    >
      ...
      <div style={{ height: "1px", background: "rgba(255,255,255,.06)", margin: "0 0 4px" }} />
      {items.map((item, i) => (
        <button
          key={i}
          onClick={item.action}
          style={{
            ...
            padding: "8px 10px",
            textAlign: "left",
            color: item.danger ? "rgba(251,100,100,.9)" : "rgba(255,255,255,.75)",
          ...
          onMouseEnter={e => {
            const el = e.currentTarget as HTMLElement;
            el.style.background = item.danger ? "rgba(251,100,100,.1)" : "rgba(255,255,255,.06)";
            el.style.color = item.danger ? "rgba(255,110,110,1)" : "rgba(255,255,255,.95)";
          }}
          onMouseLeave={e => {
            const el = e.currentTarget as HTMLElement;
            el.style.background = "transparent";
            el.style.color = item.danger ? "rgba(251,100,100,.9)" : "rgba(255,255,255,.75)";
          }}
        >
          <span style={{ opacity: item.danger ? 0.85 : 0.55, display: "flex", alignItems: "center", flexShrink: 0 }}>
            <item.Icon />
          </span>
          {item.label}
        </button>
    ...
    <>
      ...
      {ctxMenu && (
        <ContextMenu
          ...
          onChangeIcon={() =>
            setIconPicker({ x: ctxMenu.x, y: ctxMenu.y, id: ctxMenu.id })
          }
        ...
        <div ref={bellAnchorRef} style={{
          ...
          {!collapsed && (
            <span style={{
              ...
              color: C.t0, fontFamily: MONO, userSelect: "none", flex: 1,
            }}>STACKBOX</span>
          )}
          {/* Notification bell */}
          <NotificationBell
            runboxes={runboxes}
            onOpen={() => setNotifPanelOpen(v => !v)}
          />
          ...
          {runboxes.map(rb => {
            ...
            if (collapsed) return (
              <div key={rb.id} title={rb.name}
                ...
                style={{
                  ...
                  justifyContent: "center", fontSize: rbIcon ? 18 : 12,
                  fontWeight: 700, fontFamily: SANS, transition: "all .12s",
            ...
            return (
              <div key={rb.id}
                ...
                onContextMenu={e => handleContextMenu(e, rb.id)}
                style={{
                  marginBottom: 3, cursor: "pointer", borderRadius: 10,
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                  {rbIcon ? (
                    <span style={{ fontSize: 16, lineHeight: 1, flexShrink: 0, userSelect: "none" }}>
                      {rbIcon}
                    </span>
                  ) : (
                    <span style={{
