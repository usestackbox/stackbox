...
pub async fn start(app_handle: Arc<AppHandle>, db: Db, sessions: SessionMap) {
    let db_arc = db.clone();

    let memory_routes = routes::memory::router(db_arc.clone(), sessions.clone());

    // Pass sessions into McpState so agent names are resolved on memory_write
