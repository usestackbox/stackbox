            ...
            move |headers: HeaderMap, body: Bytes| {
                ...
                async move {
                    ...
                    let raw = {
                        ...
                        let stripped = if bytes.starts_with(&[0xEF, 0xBB, 0xBF]) { &bytes[3..] } else { bytes };
                    ...
                    let (runbox_id, content, branch, commit_type, tags, agent_name_body):
                        ...
                        if let Ok(v) = serde_json::from_str::<serde_json::Value>(&raw) {
                            ...
                            if map.contains_key("content") {
                                (
                                    map.get("runbox_id").cloned().unwrap_or_else(|| "__global__".into()),
                                    map.get("content").cloned().unwrap_or_default(),
                                    map.get("branch").cloned().unwrap_or_else(|| "main".into()),
                                    map.get("commit_type").cloned().unwrap_or_else(|| "memory".into()),
                                    map.get("tags").cloned().unwrap_or_default(),
                                    map.get("agent_name").cloned().unwrap_or_default(),
                                )
                            } else if raw.starts_with("content=") {
                                ("__global__".into(), raw["content=".len()..].to_string(),
                                 "main".into(), "memory".into(), "".into(), "".into())
                            } else {
                                ("__global__".into(), raw.clone(),
                                 "main".into(), "memory".into(), "".into(), "".into())
                            }
                        };
                            ...
                            if let Ok(map) = sessions.lock() {
                                map.get(&auth_sid)
                                    .map(|s| {
                                        ...
                                        if name == "Shell" { "".to_string() } else { name.to_string() }
                                    })
                                    .unwrap_or_default()
                            } else { String::new() }
                        } else { String::new() }
                    ...
                    match memory::memory_add_full(
                        ...
                        &branch, &commit_type, &tags, "", &agent_name,
                    ).await {
                        Ok(_) => {
