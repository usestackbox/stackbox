...
pub fn migrate(conn: &Connection) -> Result<()> {
    ...
    //
    // Five event types only:
    //   AgentSpawned      — agent process entered the runbox
    //   CommandExecuted   — command ran in workspace
    //   CommandResult     — command exit code + duration
    //   FileChanged       — file modified (debounced)
    //   WorkspaceSnapshot — git checkpoint (commit or auto-snapshot)
    //
    // Rules:
    //   - NEVER update rows
    //   - NEVER add new event types casually
    //   - payload_json is always flat (no nesting, no arrays)
    conn.execute_batch("
    ...
    // New code writes to workspace_events only.
    conn.execute_batch("
